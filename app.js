...
function renderHome(root){
  ...
  if(d.home && d.home.hero && d.home.hero.image){
    ...
    hero.innerHTML = `
      ...
      <div class="overlay">
        <h2>${h.overlayTitle || d.appName || 'Saving The Great Salt Lake'}</h2>
        <p>${h.overlaySubtitle || d.mission || ''}</p>
      </div>
  ...
  // Daily highlight (uses posts)
  const idx = dayIndex() % ((d.posts&&d.posts.length) || 1);
  const todays = (d.posts||[])[idx] || {};
  const canShare = !!navigator.share;
  const shareBtn = canShare ? `<button class="btn secondary" id="shareBtn">Share this app</button>` : '';
  root.appendChild(card('Today’s Highlight', `<h3>${todays.title||'Learn more inside'}</h3>${todays.image?`<img src="${todays.image}" alt="${todays.alt||''}" />`:''}${todays.imageCredit?`<div class="credit">${todays.imageCredit}</div>`:''}<p>${todays.body||''}</p>${shareBtn}`));
  if(canShare){ $('#shareBtn')?.addEventListener('click', ()=>{
    navigator.share({title: d.appName, text: todays.title||d.appName, url: location.href}).catch(()=>{});
  })}

  // HOME GALLERY (optional)
  if(d.home && Array.isArray(d.home.gallery) && d.home.gallery.length){
    const wrap=document.createElement('div'); wrap.className='card';
    wrap.innerHTML = `<div class="kicker">Gallery</div><div class="gallery"></div>`;
    const g=$('.gallery',wrap);
    d.home.gallery.forEach(ph=>{
      const tile=document.createElement('div'); tile.className='tile';
      tile.innerHTML = `<img src="${ph.src}" alt="${ph.alt||''}"/>${ph.credit?`<div class="credit">${ph.credit}</div>`:''}`;
      g.appendChild(tile);
    });
...
function renderSlides(root){
  ...
  if(!slides.length){
    root.appendChild(card('Slides', `<p>No slide decks yet. Add items to <code>data/content.json → "slides"</code>.</p>`));
    return;
  }
  slides.forEach(s=>{
    let embed='';
...
function renderPress(root){
  ...
  if(!items.length){
    root.appendChild(card('Press', `<p>No news releases yet. Add items to <code>data/content.json → "press"</code>.</p>`));
    return;
  }
  items.forEach(p=>{
    let embed='';
